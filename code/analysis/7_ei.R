...
base_path <- "../../data"
ga_agg_path <- file.path(base_path, "agg_results_ei.csv")
...
prec_res_s <- prec_res_s[prec_res_s$pct_novote>=0,]; dim(prec_res_s)

# Final results tibble
race <- c("White", "Black")
candidates <- c("Abrams", "Kemp")
ei <- c("Iterative", "RxC")
methods <- c("Self-Reported", "BISG", "CVAP")
total <- length(race) * length(candidates) * length(methods) * length(ei)
# Create final columns
race <- rep(race, total / 2)
candidates <- rep(rep(race, each = 2), total / 4)
ei <- rep(rep(ei, each = 4), total / 8)
methods <- rep(methods, each = 8)
